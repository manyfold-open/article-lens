...
test('every stage leaves the reserves the later stages need', () => {
  ...
  // stage 1 stops early enough for one Context call plus Synthesiser.
  assert.equal(stageDeadline(budget, 'stage1'), budget.deadlineAt - 150_000 - 90_000);
  // Context only has to leave room for Synthesiser.
  assert.equal(stageDeadline(budget, 'ctx'), budget.deadlineAt - 90_000);
  // Synthesiser is last, so it owns the rest of the run.
...
test('later stages always get a strictly later deadline', () => {
  ...
  assert.ok(stage1 < ctx, 'stage 1 must not be able to spend Context reserve');
  assert.ok(ctx < synth, 'Context must not be able to spend Synthesiser reserve');
});

test('Debate Verdict widens the verdict reserve at stage 1 expense', () => {
  ...
  // Synthesiser's floor is untouched by the debate flag.
