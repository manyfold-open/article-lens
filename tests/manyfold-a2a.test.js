...
test('refuses to start a call once the stage budget is gone', async t => {
  ...
  const error = await manyfold
    .callMfAgent(environment(), 'agt_no_budget', 'prompt', {
      attempts: 2,
  ...
  assert.equal(requestCount, 0, 'a peer must not be called with no budget left');
});

test('blames the budget, not the peer, when the deadline shortened the attempt', async t => {
  // The deadline is closer than the 240s timeout, so this failure happened
  // inside a window the orchestrator itself cut short. Reporting it as a peer
  ...
  const error = await manyfold
    .callMfAgent(environment(), 'agt_squeezed', 'prompt', {
      attempts: 1,
...
test('a peer failure with budget to spare stays a peer failure', async t => {
  mockFetch(t, async url => {
    if (String(url).includes('/token')) {
      return jsonResponse({
        token: 'peer-token',
  ...
  const error = await manyfold
    .callMfAgent(environment(), 'agt_real_failure', 'prompt', {
      attempts: 1,
      timeoutMs: 10_000,
