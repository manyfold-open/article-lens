...
// per-attempt timeout and one retry, so 小導 and 統整 could be left with a few
...
// quickly, 小導 and 統整 still get their full per-call timeout, because their
// own deadline is the run deadline minus only what still comes after them.

export type Stage = 'stage1' | 'ctx' | 'synth'

// One 小導 call plus room for a transient retry.
const CTX_RESERVE_MS = 150_000
// 辯論裁定 runs 正方/反方 in parallel and then adjudicates, so it needs a
// second sequential round.
const CTX_DEBATE_RESERVE_MS = 330_000
// 統整 is a single call that legitimately runs long, but it only prunes an
