...
import type { AgentName, Env, SSEEvent } from '../schema'
...
export async function callMfAgent(
  ...
  emitTrace(
    ...
    callId,
    'input',
    'Actual input sent through Manyfold A2A',
    prompt,
  )
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (remainingBudget < MIN_ATTEMPT_BUDGET_MS) {
      ...
      emitTrace(
        ...
        callId,
        'error',
        'Orchestration time budget exhausted',
        lastErr.message,
        attempt + 1,
        false,
      )
    ...
    try {
      emitTrace(
        ...
        'progress',
        attempt > 0
          ? `Attempt ${attempt + 1}: reconnecting`
          : 'Minting a short-lived A2A credential',
        undefined,
        attempt + 1,
      )
      emitTrace(
        ...
        callId,
        'progress',
        'Stream opened; waiting for the agent',
        undefined,
        attempt + 1,
      )
      ...
      emitTrace(
        ...
        callId,
        'progress',
        `Retrying in ${Math.ceil(waitMs / 100) / 10}s`,
        undefined,
        attempt + 1,
      )
